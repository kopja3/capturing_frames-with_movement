...
settings = Settings()
app = FastAPI(title="Bird Detection API", version="1.0.0")
app.add_middleware(
    CORSMiddleware,
...
class MotionGate:
    ...
    def detect(self, frame: np.ndarray) -> tuple[bool, list[dict[str, int]]]:
        h, w = frame.shape[:2]
...
def analyze_frame(frame: np.ndarray, motion_gate: MotionGate) -> dict[str, Any]:
    ...
    if settings.enable_motion_gate:
        motion_detected, motion_boxes = motion_gate.detect(frame)
