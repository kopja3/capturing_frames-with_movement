...
class Settings:
    ...
    motion_min_area: int = int(os.getenv("MOTION_MIN_AREA", "900"))
    enable_motion_gate: bool = os.getenv("ENABLE_MOTION_GATE", "1") != "0"
    allow_origins: str = os.getenv("ALLOW_ORIGINS", "*")


settings = Settings()
app = FastAPI(title="Bird Detection API With Foreground Mask", version="1.1.0")
app.add_middleware(
    CORSMiddleware,
...
class MotionGate:
    ...
    def detect(self, frame: np.ndarray) -> tuple[bool, list[dict[str, int]], np.ndarray]:
        h, w = frame.shape[:2]
        ...
        mask = self.subtractor.apply(resized)
...
def analyze_frame(frame: np.ndarray, motion_gate: MotionGate) -> dict[str, Any]:
    ...
    if settings.enable_motion_gate:
        motion_detected, motion_boxes, motion_mask = motion_gate.detect(frame)
